...
contract Parameterizer {
    ...
    uint public maxChangePct = 10;

    uint public MAXIMUM_CHANGE_PCT = 10;
    ...
    */
    function Parameterizer(
        ...
        params[keccak256("minDeposit")] = _parameters[0];

        // minimum deposit to propose a reparameterization
        params[keccak256("pMinDeposit")] = _parameters[1];

        // period over which applicants wait to be whitelisted
        params[keccak256("applyStageLen")] = _parameters[2];

        // period over which reparmeterization proposals wait to be processed
        params[keccak256("pApplyStageLen")] = _parameters[3];

        // length of commit period for voting
        params[keccak256("commitStageLen")] = _parameters[4];

        // length of commit period for voting in parameterizer
        params[keccak256("pCommitStageLen")] = _parameters[5];

        // length of reveal period for voting
        params[keccak256("revealStageLen")] = _parameters[6];

        // length of reveal period for voting in parameterizer
        params[keccak256("pRevealStageLen")] = _parameters[7];

        // percentage of losing party's deposit distributed to winning party
        params[keccak256("dispensationPct")] = _parameters[8];

        // percentage of losing party's deposit distributed to winning party in parameterizer
        params[keccak256("pDispensationPct")] = _parameters[9];

        // type of majority out of 100 necessary for candidate success
        params[keccak256("voteQuorum")] = _parameters[10];

        // type of majority out of 100 necessary for proposal success in parameterizer
        params[keccak256("pVoteQuorum")] = _parameters[11];

        // maximum percentage change allowed in a single proposal in parameterizer
        // params[keccak256("maxChangePct")] = _parameters[12];
    }
    ...
    */
    function proposeReparameterization(string _name, uint _value) public returns (bytes32) {
        uint deposit = params[keccak256("pMinDeposit")];
        ...
        require(checkValidity(_value, get(_name)));
        ...
        proposals[propID] = ParamProposal({
            appExpiry: now.add(params[keccak256("pApplyStageLen")]),
            challengeID: 0,
            deposit: deposit,
            name: _name,
            owner: msg.sender,
            processBy: now.add(params[keccak256("pApplyStageLen")])
                .add(params[keccak256("pCommitStageLen")])
                .add(params[keccak256("pRevealStageLen")])
                .add(PROCESSBY),
            value: _value
        });
    ...
    function checkValidity(uint _oldValue, uint _newValue) internal view returns (bool _valid) {
        uint tempValue = 0;

        if (_newValue >= _oldValue) {
            tempValue = _newValue.sub(_oldValue);
        } else {
            tempValue = _oldValue.sub(_newValue);
        }

        require(tempValue <= _oldValue.mul(maxChangePct).div(100));
    ...
    */
    function challengeReparameterization(bytes32 _propID) public returns (uint challengeID) {
        ...
        uint pollID = voting.startPoll(
            params[keccak256("pVoteQuorum")],
            params[keccak256("pCommitStageLen")],
            params[keccak256("pRevealStageLen")]
        );

        challenges[pollID] = Challenge({
            challenger: msg.sender,
            rewardPool: SafeMath.sub(100, params[keccak256("pDispensationPct")]).mul(deposit).div(100),
            stake: deposit,
            resolved: false,
            winningTokens: 0
        });
    ...
    */
    function processProposal(bytes32 _propID) public {
        ...
        if (canBeSet(_propID)) {
            ...
            params[keccak256(prop.name)] = prop.value;
        ...
        assert(params[keccak256("dispensationPct")] <= 100);
        assert(params[keccak256("pDispensationPct")] <= 100);

        // verify that future proposal appExpiry and processBy times will not overflow
        now.add(params[keccak256("pApplyStageLen")])
            .add(params[keccak256("pCommitStageLen")])
            .add( params[keccak256("pRevealStageLen")])
            .add(PROCESSBY);
    ...
    */
    function resolveChallenge(bytes32 _propID) private {
        ...
        if (voting.getTotalNumberOfTokensForWinningOption(prop.challengeID)== 0) { // The challenge failed
