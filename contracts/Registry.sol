...
contract Registry {
    ...
    event _Application(bytes32 indexed listingHash, uint deposit, uint appEndDate, bytes32 data, address indexed applicant);
    ...
    event _ApplicationUpdated(bytes32 indexed listingHash);
    event _ListingRemoved(bytes32 indexed listingHash);
    event _ListingWithdrawn(bytes32 indexed listingHash);
    event _TouchAndRemoved(bytes32 indexed listingHash);
    event _ChallengeFailed(bytes32 indexed listingHash, uint indexed challengeID, bytes32 ipfsLink, uint rewardPool, uint totalTokens);
    event _ChallengeSucceeded(bytes32 indexed listingHash, uint indexed challengeID, bytes32 ipfsLink, uint rewardPool, uint totalTokens);
    ...
    */
    function Registry(address _token, address _voting, address _parameterizer, string _name) public {
    ...
    */
    function resolveChallenge(bytes32 _listingHash) private {
        ...
        if (voting.isPassed(challengeID)) {
            ...
            emit _ChallengeFailed(_listingHash, challengeID, challenges[challengeID].ipfsLink, challenges[challengeID].rewardPool, challenges[challengeID].totalTokens);
        }
        // Case: challenge succeeded or nobody voted
            ...
            emit _ChallengeSucceeded(_listingHash, challengeID, challenges[challengeID].ipfsLink, challenges[challengeID].rewardPool, challenges[challengeID].totalTokens);
        }
    }
    ...
    */
    function resetListing(bytes32 _listingHash) private {
        ...
        if (listing.whitelisted) {
            ...
            emit _ApplicationUpdated(_listingHash);
        }

        // Deleting listing to prevent reentry
        address owner = listing.owner;
        uint unstakedDeposit = listing.unstakedDeposit;
        listings[_listingHash].ipfsLink = challenges[listing.challengeID].ipfsLink;
